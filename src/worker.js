...
function QRCodeModel(typeNumber, errorCorrectionLevel) {
  ...
  this.determineType = () => {
      let length = this.dataList.reduce((acc, data) => acc + data.data.length * 8, 0);
  ...
  const QRUtil = {
      ...
      getRSBlocks:(t,e)=>{const r=[[1,26,19,1,26,19],[1,26,16,1,26,16],[1,26,13,2,26,13],[1,26,9,2,26,9]];const o=r[e];const n=[];for(let i=0;i<o[0];i++)n.push({totalCount:o[1],dataCount:o[2]});return n;},
      ...
      getCapacity: (type, ecl) => { const T = type * 4 + 17; const err = [[1, 26, 19], [1, 26, 16], [1, 26, 13], [1, 26, 9]][ecl][2]; return Math.floor(T*T/8) - err*3; } // simplified
  };
  ...
  QRUtil.createBytes = (buffer, rsBlocks) => buffer.buffer;
}
